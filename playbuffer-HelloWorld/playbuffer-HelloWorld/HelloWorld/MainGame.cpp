...
// Declares all the functions that are used
...
// Gets called each frame to handle player input
void HandlePlayerControls() {
...
// Deals with the fan AI, by moving up and down and spawning tools
void UpdateFan() {
	GameObject& obj_fan = Play::GetGameObjectByType(TYPE_FAN);
	// Spawns tools using a call from random rolls
	...
	// Spawns coins using a call from random rolls
...
// Tools that are used to attack the agent
void UpdateTools() {
	GameObject& obj_agent8 = Play::GetGameObjectByType(TYPE_AGENT8);
	...
	for (int id : vTools){
		...
		// Handles collision with the agent
...
// The coins which gives points and spawns stars when collided with the agent
void UpdateCoinsAndStars() {
	GameObject& obj_agent8 = Play::GetGameObjectByType(TYPE_AGENT8);
...
void UpdateLasers() {
	...
	for (int id_laser : vLasers) {
		...
		// When the laser collides with a tool
		...
		// When the laser collides with a coin
		for (int id_coin : vCoins) {
			GameObject& obj_coin = Play::GetGameObject(id_coin);
...
// Handles obj that are going to be destroyed
void UpdateDestroyed() {
	...
	for (int id_dead : vDead) {
		...
		// Creates a flashing affected before being destroyed
		if (obj_dead.frame % 2) {
...
//Handles the agent's different states
void UpdateAgent8() {
	GameObject& obj_agent8 = Play::GetGameObjectByType(TYPE_AGENT8);
	
	switch (gameState.agentState)
	{
	// When game starts the first state
	case STATE_APPEAR:
		...
		break;
	//Gives access to the player after the agent stops moving
	case STATE_HALT:
		...
		break;
	// When the player is in control of the agent.
	case STATE_PLAY:
		HandlePlayerControls();
		break;
	// When agent is dead deals with the ending.
	case STATE_DEAD:
		obj_agent8.acceleration = { -0.3f, 0.5f };
		obj_agent8.rotation += 0.25;

		// For respawning the agent
		if (Play::KeyPressed(VK_SPACE) == true) {
			...
			// Deletes the tools to make the level easier at the start
